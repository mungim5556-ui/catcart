...
export type Screen = 'title' | 'controls' | 'select' | 'pause' | 'none';

export interface MenuHandlers {
  /** Cat highlighted on the select screen changed (live preview). */
  onPreview(catIndex: number): void;
  onStart(catIndex: number, difficulty: Difficulty): void;
  onResume(): void;
  onRestart(): void;
  onQuit(): void;
}
...
function loadPrefs(): { cat: number; diff: number } {
  try {
    const p = JSON.parse(localStorage.getItem(PREFS_KEY) ?? '{}');
    return { cat: p.cat ?? 0, diff: p.diff ?? 1 };
  } catch {
    return { cat: 0, diff: 1 };
  }
}
...
export class Menus {
  ...
  key(code: string): boolean {
    ...
    switch (code) {
      ...
      case 'NumpadEnter':
      case 'Space':
        this.act(this.screen === 'select' ? 'start' : buttons[this.focus].action);
        return true;
      case 'Escape':
      case 'KeyP':
  ...
  private act(action: string): void {
    ...
    switch (action) {
      ...
      case 'start':
        this.savePrefs();
        this.show('none');
        return this.h.onStart(this.cat, DIFFICULTIES[this.diff]);
      case 'resume':
        this.show('none');
  ...
  private savePrefs(): void {
    try {
      localStorage.setItem(PREFS_KEY, JSON.stringify({ cat: this.cat, diff: this.diff }));
    } catch {
      /* ignore */
    }
  }
  ...
  private render(): void {
    document.body.classList.toggle('in-menu', this.screen === 'title' || this.screen === 'controls' || this.screen === 'select');
        ...
        this.root.innerHTML = `
          ...
          <div class="select-bar">
            <div class="cat-pick">
              ...
              <button data-action="next" class="arrow" aria-label="다음">▶</button>
            </div>
            <div class="diff">${diffs}</div>
            <button data-action="start" class="go">출발! 🏁</button>
            <p class="hint"><kbd>←</kbd><kbd>→</kbd> 고양이 · <kbd>↑</kbd><kbd>↓</kbd> 난이도 · <kbd>Enter</kbd> 출발 · <kbd>Esc</kbd> 뒤로</p>
          </div>`;
        return;
      }
      case 'pause':
